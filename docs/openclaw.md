...
Open the one-use browser link printed by `pnpm gateway`. The credential is kept only in browser memory; after reloading or expiry, restart the application with the same state directory and open its new link. Do not reuse an already consumed link. Restarting preserves local plans, evidence, and review history.
